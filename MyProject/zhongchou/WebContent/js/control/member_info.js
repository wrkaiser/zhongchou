	...
	<div class="member-group" data-row="<% row %>">\
		<div class="form-list">\
			...
			<div class="inp add-list clearfix">\
				<div class="fl">\
					<input wx-upload-type="" type="file" wx-upload="Upload" name="member_avatar<% row %>_wxupload" wx-upload-assign="member_avatar<% row %>=data.fileUrl" wx-upload-set="member_avatar<% row %>_img=data.fileUrl" hidefocus="true">\
					<div class="add-btn"></div>\
				</div>\
				<div class="fl">\
					<em class="tag">＊文件大小请勿超过8MB</em>\
					<input type="hidden" wx-validator-rule="required" class="picarr" name="member_avatar" id="member_avatar<% row %>" value="<% photo %>">\
					</div>\
			</div>\
...
\
		<div class="form-list">\
			...
			<div class="inp">\
				<div class="clearfix">\
					<input wx-validator-rule="required" name="member_name" wx-validator-param="" id="member_name<% row %>" type="text" placeholder="" value="<% name %>">\
		...
		<div class="form-list">\
			...
			<div class="inp">\
				<div class="clearfix">\
					<input wx-validator-rule="required" name="title" wx-validator-param="" id="title<% row %>" type="text" placeholder="" value="<% title %>">\
				</div>\
...
\
		...
		<div class="form-list">\
			...
			<div class="inp">\
				<textarea wx-validator-rule="" name="member_profile" wx-validator-param="" id="member_profile<% row %>" type="text" placeholder="" ><% intro %></textarea>\
			</div>\
	...
	$.fn.member_info = function(options){
		...
		if(options.jsonData != undefined){
			try{
				...
				options.jsonData=options.jsonData.replaceAll('\\"\\[','[');
				options.jsonData=options.jsonData.replaceAll('\\]\\"',']');
				options.jsonData=options.jsonData.replaceAll('\\"\\"\\"\\"','[]');
				try{
					this.data = $.parseJSON(options.jsonData);
		...
		function add_js(i, row_control, rowData){
			...
			window["member_avatar"+i+"_wxupload"] = function(data){
				...
				$("#member_avatar" + i + "_picul").find("img").attr("src", data.img);
				$("#member_avatar" + i + "_span").show();	
				$("#member_avatar" + i).val(data.img);
			};
