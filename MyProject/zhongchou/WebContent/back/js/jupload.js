...
function addFile(image) { 
    var el = $('<li class="post file" data-post-id="0">'
         + '<input type="hidden" value="0" name="saids">'
		 + '<input type="hidden" value="' + image.url + '" name="saimgs">'
		+ '<a class="thumb" href="' + image.url + '" target="_blank" title="点击查看原图，拖动排序" style="background-image:url(' + image.thm_url + ')"><img src="'+image.thm_url+'"></img></a>'//this.thumb
		//+ '<p class="file-name">' + image.title + '<span class="file-info">' + (json ? json.warning || json.message : '') + '</span></p>'//this.info
		+ '<dl class="form data">'
			+ '<dt>标题</dt><dd class="title"><input type="text" class="text" placeholder="无标题请留空" name="sanames" value="' + (image.title || '') + '" /></dd>' //this.title
			+ '<dt>描述</dt><dd class="description"><textarea name="sadescbs">' + image.title + '</textarea></dd>'
    ...
    //+ '<li class="file-size"></li>'//用flash或者html5一定有size
		+ '<li class="delete"><a class="ir" said="0">删除</a></li>'
